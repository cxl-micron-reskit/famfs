...
F_SMALL=test10
expect_good "${CLI[@]}" creat -r -s 8192 -S 10 "$MPT/$F_SMALL" \
           -- "creat $F_SMALL"
expect_good "${CLI[@]}" verify -S 10 -f "$MPT/$F_SMALL" \
           -- "verify $F_SMALL after replay"

F_8M=bigtest0
expect_good "${CLI[@]}" creat -v -r -S 42 -s 0x800000 "$MPT/$F_8M" \
           -- "creat $F_8M"
expect_good "${CLI[@]}" verify -S 42 -f "$MPT/$F_8M" \
           -- "$F_8M mismatch"

expect_good "${CLI[@]}" cp -h -- "cp -h should succeed"

expect_good "${CLI[@]}" cp -vvv "$MPT/$F_8M" "$MPT/${F_8M}_cp" \
           -- "cp $F_8M"
expect_good "${CLI[@]}" verify -S 42 -f "$MPT/${F_8M}_cp" \
           -- "verify ${F_8M}_cp"
...
expect_good "${CLI[@]}" mkdir "$PFX/./foo/foo//bar/baz" \
           -- "mkdir relpath 4"
expect_fail "${CLI[@]}" mkdir "$PFX/./foo/./foo//bar/baz" \
           -- "mkdir relpath exists should fail"
cd - >/dev/null || true
...
expect_good "${CLI[@]}" mkdir -pv "$MPT/AAAA/BBBB/CCC" \
           -- "mkdir -p 2"
expect_good "${CLI[@]}" mkdir -pv "$MPT/A/B/C/w/x/y/z" \
           -- "mkdir -p 3"

sudo chmod 0666 "$MPT/A"
cd "$MPT/.." || fail "cd $MPT/.. failed (mkdir -p section)"
pwd
expect_good "${CLI[@]}" mkdir -p "$PFX/A/x/y/z"      -- "mkdir -p 4"
expect_good "${CLI[@]}" mkdir -p "$PFX/./A/x/y/z"    -- "mkdir -p 5"
cd - >/dev/null || true

expect_fail "${CLI[@]}" mkdir -pv "$MPT/${F_8M}/foo/bar/baz/bing" \
           -- "mkdir -p with a file in path should fail"

if [[ "$FAMFS_MODE" == "fuse" ]]; then
    # turn up log debug
    expect_good sudo curl \
        --unix-socket "$(scripts/famfs_shadow.sh "$MPT")/sock" \
        -X POST -d 7 http://localhost/log_level \
        -- "set log_level on fuse"
fi

expect_fail "${CLI[@]}" mkdir -pvvv "$MPT/a/y/../../../.." \
           -- "mkdir -p ../../../.. ascended out of famfs"

if [[ "$FAMFS_MODE" == "fuse" ]]; then
    # Dump the icache to the log
    expect_good sudo curl \
        --unix-socket "$(scripts/famfs_shadow.sh "$MPT")/sock" \
        http://localhost/icache_dump \
        -- "icache_dump"
fi
...
expect_good "${CLI[@]}" cp "$MPT/$F_8M" "$MPT/subdir/${F_8M}_cp0" \
           -- "overcopy should work"
expect_fail "${CLI[@]}" cp "$MPT/$F_SMALL" "$MPT/subdir/${F_8M}_cp0" \
           -- "overcopy wrong size should fail"

#
# Copy stuff that is invalid
#
expect_fail "${CLI[@]}" cp /tmp/nonexistent_file "$MPT/" \
           -- "cp nonexistent should fail"
expect_fail "${CLI[@]}" cp /dev/zero "$MPT/" \
           -- "cp /dev/zero (cardev) should fail"
...
expect_good "${CLI[@]}" cp "$MPT/$F_8M" "$MPT/dirtarg" \
           -- "cp to dir $F_8M"
expect_good "${CLI[@]}" verify -S 42 -f "$MPT/dirtarg/$F_8M" \
           -- "verify dirtarg/$F_8M"
...
FMODE="600"
# UID is the bash builtin for user id
GID=$(id -g)

cd "$MPT/.." || fail "cd $MPT/.. failed (cp wildcard section)"
DEST=A/B/C/w/x/y/z
expect_good "${CLI[@]}" cp -m "$FMODE" -u "$UID" -g "$GID" \
           "$MPT/subdir/"* "$MPT/$DEST" \
           -- "cp wildcard set to directory should succeed"
...
FMODE_OUT="$(sudo stat --format='%a' "$FILE")"
if [[ "$FMODE" != "$FMODE_OUT" ]]; then
    fail "cp -m err $FMODE ${FMODE_OUT} after remount"
fi
UID_OUT="$(sudo stat --format='%u' "$FILE")"
if [[ "$UID" != "$UID_OUT" ]]; then
...
#
expect_good "${CLI[@]}" cp "$MPT/subdir/"* "$MPT/dirtarg" \
           -- "cp wildcard set to directory should succeed"
...
#
cd "$MPT" || fail "cd $MPT failed (verify deep dir section)"
...
# This directory will cause cp * from dirtarg/ to return non-zero since it
...
#
cd "$MPT/.." || fail "cd $MPT/.. failed (relative wildcard cp section)"
...
expect_good "${CLI[@]}" cp "$MPT/dirtarg/${F_8M}_cp1" "$MPT/smalldir" -- "cp to smalldir 1"
expect_good "${CLI[@]}" mkdir "$MPT/smalldir2" -- "failed to create smalldir2"

expect_fail "${CLI[@]}" cp -r "$MPT/smalldir" "$MPT/$F_8M" \
           -- "recursive copy to file destination should fail"
expect_fail "${CLI[@]}" cp "$MPT/smalldir/"* "$MPT/$F_8M" \
           -- "wildcard multi-file copy to file destination should fail"

#
# cp -r with absolute paths
#
expect_good "${CLI[@]}" cp -rv "$MPT/smalldir/"* "$MPT/smalldir2" \
           -- "recursive copy 0"
expect_good "${CLI[@]}" cp -c -rv "$MPT/smalldir/"* "$MPT/smalldir2" \
           -- "recursive compare 0"
expect_good sudo diff -r "$MPT/smalldir" "$MPT/smalldir2" \
           -- "diff -r smalldir smalldir2"

# deface one of the files and re-compare: should be error
overwrite_page "$MPT/smalldir2/${F_8M}_cp1" 0
expect_fail sudo diff -r "$MPT/smalldir" "$MPT/smalldir2" \
           -- "cp -cr smalldir smalldir2"
expect_fail "${CLI[@]}" cp -c -r "$MPT/smalldir/"* "$MPT/smalldir2" \
           -- "recursive compare 1"

expect_good "${CLI[@]}" cp -r "$MPT/A" "$MPT/A-prime" \
           -- "cp -r A A-prime"
expect_good sudo diff -r "$MPT/A" "$MPT/A-prime" \
           -- "diff -r A A-prime"

#
# cp -r with relative paths
#
cd "$MPT/.." || fail "cd $MPT/.. failed (cp -r relative section)"
expect_good "${CLI[@]}" cp -r "$PFX/A" "$PFX/A-double-prime" \
           -- "cp -r A A-double-prime"
sudo diff -r "$PFX/A" "$PFX/A-double-prime"
cd - >/dev/null || true

# Bad cp -r
expect_fail "${CLI[@]}" cp -r "$MPT/A" "$MPT/bar/foo" \
           -- "cp -r to bogus path should fail"
expect_fail "${CLI[@]}" cp -r "$MPT/A" "$MPT/$F_8M" \
           -- "cp -r to file"
expect_fail "${CLI[@]}" cp -r "$MPT/A" "$MPT/$F_8M/foo" \
           -- "cp -r to path that uses file as dir"

expect_good "${CLI[@]}" cp "$MPT/A/B/C/w/x/y/z/"* "$MPT" \
           -- "cp valid wildcard to mount pt dir should succeed"
...
expect_good sudo cmp "$MPT/$F_8M" "$MPT/${F_8M}_cp" \
           -- "copies should match"
