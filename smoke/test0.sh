...
expect_fail "${CLI[@]}" creat -S 1 -r -M "$MPT/notcreated,22" \
           -- "seed and multi incompatible"
...
expect_good "${CLI[@]}" verify --multi "$MPT/seeded,42" \
           -- "seeded file should verify"
...
expect_fail "${CLI[@]}" verify --multi "$MPT/test1" \
           -- "no comma in verify multi arg"
expect_fail "${CLI[@]}" verify --multi "$MPT/test1,1,extra" \
           -- "extra fields in multi"
expect_good "${CLI[@]}" verify \
           --multi "$MPT/test1,1" \
           --multi "$MPT/test2,2" \
           --multi "$MPT/test3,3" \
           -- "verify multi with good seeds should succeed"
expect_fail "${CLI[@]}" verify \
           --multi "$MPT/test1,9" \
           --multi "$MPT/test2,2" \
           --multi "$MPT/test3,3" \
...
expect_good "${CLI[@]}" creat -r -s 4096 -S 99 "$MPT/test1" \
           -- "Create to re-init existing file should succeed"
expect_good "${CLI[@]}" creat -s 4096 "$MPT/test1"  \
           -- "Recreate with same size should succeed"
expect_fail "${CLI[@]}" creat -s 8192 "$MPT/test1"  \
           -- "Recreate with different size should fail"

# Put back data for seed=1 because we're gonna check it later
expect_good "${CLI[@]}" creat -r -s 4096 -S 1 "$MPT/test1" \
           -- "Create to re-init existing file should succeed"

# Create outside famfs should fail
expect_fail "${CLI[@]}" creat -r -s 4096 -S 1 /tmp/test1 \
           -- "Create should fail if file exists"

# Create an empty file should fail
expect_fail "${CLI[@]}" creat -r -s 0 -S 1 "$MPT/emptyfile" \
           -- "Create empty file should fail"

# Test creat mode/uid/gid options
FMODE="600"
uid=$(id -u)
gid=$(id -g)
expect_good "${CLI[@]}" creat -s 0x100000 -r -m "$FMODE" -u "$uid" -g "$gid" "$MPT/testmode0" \
           -- "creat with mode/uid/gid"
...
#
DIRPATH="$MPT/z/y/x/w"
expect_good "${CLI[@]}" mkdir -p -m "$FMODE" -u "$uid" -g "$gid" "$DIRPATH" \
           -- "mkdir -p with mode/uid/gid"
...
if [[ "$FAMFS_MODE" == "fuse" ]]; then
    expect_good sudo curl --unix-socket \
        ...
        -- "icache_stats REST query failed"
fi

# Unmount and remount
expect_good sudo "$UMOUNT" "$MPT" -- "umount should succeed"
expect_fail findmnt -rn -S "$DEV" --target "$MPT" \
           -- "famfs is still mounted at $MPT after umount attempt"

if [[ "$FAMFS_MODE" == "v1" ]]; then
    expect_good sudo mount "${RAW_MOUNT_OPTS[@]}" "$DEV" "$MPT" \
               -- "mount"
...
expect_good "${CLI[@]}" mkmeta "$DEV"            -- "mkmeta repeat should succeed"

# Replay the log, recovering the files that existed before the umount
...
echo "re-checking creat -mug after remount"
FMODE_OUT="$(sudo stat --format='%a' "$MPT/testmode0")"
if [[ "$FMODE" != "$FMODE_OUT" ]]; then
    fail "creat -m err $FMODE ${FMODE_OUT}"
fi
...
if [[ "$FAMFS_MODE" == "v1" ]]; then
    ...
    expect_fail "${CLI_NOSUDO[@]}" check "$MPT" \
               -- "famfs check without sudo should fail"
    expect_good "${CLI[@]}" check "$MPT" \
               -- "famfs check should succeed"
    expect_fail "${CLI[@]}" check "relpath" \
               -- "famfs check on relpath should fail"
    expect_fail "${CLI[@]}" check "/badpath" \
               -- "famfs check on bad path should fail"

    sudo touch "$MPT/unmapped_file"
    expect_fail "${CLI[@]}" check -vvv "$MPT" \
               -- "famfs check should fail due to unmapped file"
    sudo rm "$MPT/unmapped_file"
    expect_good "${CLI[@]}" check -v "$MPT" \
               -- "famfs check should succeed after removing unmapped file"

    expect_good "${CLI_NOSUDO[@]}" fsck -hv "$MPT" \
               -- "fsck without sudo should succeed"
fi
...
expect_good "${CLI[@]}" flush $(sudo find "$MPT" -type f -print) \
           -- "flush all files should work"
expect_fail "${CLI[@]}" flush -vv $(sudo find "$MPT" -print) \
           -- "this flush should report errors"

expect_fail "${FSCK[@]}"           -- "fsck with no args should fail"
expect_good "${FSCK[@]}" -?        -- "fsck -? should succeed"
